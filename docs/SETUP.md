...
Используйте свой аккаунт Yandex Cloud с промокодом куратора и учебный каталог.
В этом развёртывании используется каталог `default` (`b1g49hdr2hurtub7lqhs`),
а сеть, подсеть, группа безопасности и ВМ Kittygram создаются отдельно.
...
В этом проекте ему выданы `compute.editor`, `vpc.admin` и `storage.admin` на **этот каталог**.
...
В текущем развёртывании бакет `kittygram-state-mezhnun89-b1g49hdr2hurtub7lqhs`
уже создан через S3 API до первого `infra init`: ACL закрытый, versioning включён.
Ниже описан альтернативный способ создания нового бакета через `bootstrap/`.
Для существующего бакета сначала выполните `terraform import yandex_storage_bucket.state <BUCKET>`
из каталога `bootstrap` с настроенными переменными и ключами; не создавайте его повторно.
...
| `YC_FOLDER_ID` | ID учебного каталога |
